      ...
      <div className="bg-white rounded-3xl p-6 w-full max-w-sm animate-slideUp shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2">
            <Lock size={20} className="text-[#f65e1d]" /> Enter UPI PIN
          </h3>
          ...
          {[0, 1, 2, 3].map((i) => (
            <div
              ...
              className={`w-12 h-12 rounded-xl border-2 flex items-center justify-center text-xl font-bold transition-all ${
                pin.length > i ? 'border-[#f65e1d] bg-orange-50 text-[#f65e1d]' : 'border-gray-200 bg-gray-50'
              }`}
            >
              {pin.length > i ? '\u2022' : ''}
            </div>
          ...
          {[1, 2, 3, 4, 5, 6, 7, 8, 9, '', 0, 'del'].map((key, i) => (
            <button
              ...
              className={`py-3 rounded-xl text-lg font-bold border-none cursor-pointer transition-all ${
                ...
                'bg-gray-50 text-gray-800 hover:bg-orange-50 hover:text-[#f65e1d] active:scale-95'
              }`}
            >
              {key === 'del' ? '\u232B' : key}
            </button>
        ...
        <button
          ...
          className="w-full py-3.5 bg-[#f65e1d] hover:bg-[#e5531a] disabled:bg-gray-300 text-white font-bold rounded-xl border-none cursor-pointer disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2 text-base shadow-lg"
        >
