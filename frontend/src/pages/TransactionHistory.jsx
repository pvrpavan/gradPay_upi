    ...
    <div className="min-h-screen pb-20" style={{ background: `linear-gradient(135deg, ${theme.gradientFrom}, ${theme.gradientTo})` }}>
      ...
      <main className="px-4 pt-4 animate-fadeIn">
        ...
        <div className="flex items-center gap-2 mb-4 overflow-x-auto">
          <Filter size={16} style={{ color: theme.textMuted }} className="flex-shrink-0" />
          {['all', 'sent', 'received', 'deposit'].map((f) => (
            <button
              ...
              style={{
                backgroundColor: filter === f ? theme.brand : theme.bgCard,
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-xs mt-1" style={{ color: theme.textMuted }}>Start sending money to see your history</p>
          </div>
        ) : (
            ...
            {filtered.map((tx, i) => {
              ...
              return (
                <button
                  ...
                  className="w-full flex items-center justify-between rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow cursor-pointer border-none text-left"
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs" style={{ color: theme.textMuted }}>{tx.note || 'Transaction'}</p>
                      <p className="text-[10px]" style={{ color: theme.textMuted }}>{new Date(tx.timestamp).toLocaleString()}</p>
                    </div>
                  </div>
      ...
      {selectedTx && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center" onClick={() => { setSelectedTx(null); setTxDetail(null); }}>
          <div className="rounded-t-3xl p-6 w-full max-w-[430px] animate-slideUp" style={{ backgroundColor: theme.bgCard }} onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold" style={{ color: theme.text }}>Transaction Details</h3>
              <button onClick={() => { setSelectedTx(null); setTxDetail(null); }} className="bg-transparent border-none cursor-pointer" style={{ color: theme.textMuted }}>
                <X size={20} />
              </button>
            </div>
            ...
            {detailLoading ? (
              <div className="flex justify-center py-8">
                <div className="animate-spin w-6 h-6 border-2 rounded-full" style={{ borderColor: theme.brand, borderTopColor: 'transparent' }} />
              </div>
            ) : (
              <div className="space-y-4">
                {/* Amount */}
                <div className="text-center py-4 rounded-2xl" style={{ backgroundColor: theme.inputBg }}>
                  <p className="text-3xl font-bold" style={{ color: theme.text }}>{'\u20B9'}{selectedTx.amount?.toLocaleString('en-IN')}</p>
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <div key={label} className="flex items-start gap-3">
                      <Icon size={16} style={{ color: theme.brand }} className="mt-0.5 flex-shrink-0" />
                      <div>
